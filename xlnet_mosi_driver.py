...
xlnet_ex = Experiment('xlnet_transformer')
from sacred.observers import MongoObserver
from global_configs import *
url_database = conf_url_database
mongo_database_name = conf_mongo_database_name
xlnet_ex.observers.append(MongoObserver.create(url= url_database ,db_name= mongo_database_name))
...
def cnf():
    ...
    shuffle=True
    num_workers=2
    best_model_path = our_model_saving_path+str(node_index) +"xlnet_best_model.chkpt"
    loss_function="ll1"
    save_model=True
    save_mode='best'
    d_acoustic_in=0
...
def _init_fn(worker_id,_config):
    print("Data Loader Seed: ", _config["seed"])
...
def prep_for_training(num_train_optimization_steps,_config):
    ...
    model = XLNetForSequenceClassification.from_pretrained(_config["xlnet_model"],
