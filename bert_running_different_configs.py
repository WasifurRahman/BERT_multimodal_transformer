...
DB = 'sqlite:///hparam_search/' + args.dataset + '.db'

if not os.path.exists('./hparam_search/' + args.dataset + '.db'):
...
def initiate_main_experiment(_config):
    ...
    main_init_configs["num_train_epochs"] =  40
...
def objective(trial):
    optuna_configs = {}
    optuna_configs["learning_rate"] = trial.suggest_categorical("learning_rate", [1e-5,2e-5,3e-5,4e-5,5e-5])
    optuna_configs["hidden_dropout_prob"] = trial.suggest_categorical("hidden_dropout_prob", [0.1,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8])
    ...
    optuna_configs["h_audio_lstm"] =  trial.suggest_categorical("h_audio_lstm", [16,32,48,64,56,128])
    optuna_configs["h_video_lstm"] =  trial.suggest_categorical("h_video_lstm", [16,32,48,64,56,128])
    optuna_configs["fc1_out"] =  trial.suggest_categorical("fc1_out", [32,64,128,256,512])
    optuna_configs["fc1_dropout"] =  trial.suggest_categorical("fc1_dropout", [0.1,0.2,0.3,0.4,0.45,0.5,0.6])
    ...
    return run.result.info['best_val_loss']
...
if __name__ == '__main__':
    if(os.path.isdir(dataset_path)):
        ...
        if args.load_study:
            ...
            study = optuna.create_study(study_name='m-BERT hparam search', pruner=pruner, direction='minimize', storage=DB, load_if_exists=True)
